...
const OBPSAcknowledgement = ({ data, onSuccess }) => {
  ...
  let selectedArchitect = JSON.parse(sessionStorage.getItem("selectedArchitectUUID"));
  let modifiedData = {...data, selectedArchitect}
  useEffect(() => {
    try {
      ...
      data?.nocDocuments?.NocDetails.map((noc) => {
        ...
        if(data.status === "INITIATED"){
          formdata.Noc.workflow={action: "INITIATE"}
        }
        mutation.mutate(formdata, {
            onSuccess,
          });
      })
      formdata = convertToBPAObject(modifiedData);
