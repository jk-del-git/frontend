import {
  ...
  CitizenInfoLabel, Header, LinkButton, Row, StatusTable, SubmitBar, Table, CardSectionHeader, EditIcon, PDFSvg, Loader, Dropdown, TextInput, SearchIcon
...
const CheckPage = ({ onSubmit, value }) => {
  ...
  const [architectMobileInput, setArchitectMobileInput] = useState("");
  const [architectMobileSearch, setArchitectMobileSearch] = useState("");
  const [triggerSearch, setTriggerSearch] = useState(false);
  const { isLoading: hrmsLoading,
    isError,
    error: hrmsError,
    data: hrmsData,
    ...rest } = Digit.Hooks.useUserSearch(
      tenantId,
      { userName: architectMobileSearch },

      {},
      triggerSearch // search only triggers when true
    );

  useEffect(() => {
    if (hrmsData?.user?.length > 0) {
      const uuid = hrmsData.user[0].uuid;
      sessionStorage.setItem("selectedArchitectUUID", JSON.stringify(uuid));
    }
  }, [hrmsData]);

  const handleSearchArchitect = () => {
    if (!architectMobileInput) return;
    setArchitectMobileSearch(architectMobileInput);
    setTriggerSearch((prev) => !prev); // toggle to trigger the hook
  };
  // const handleSearchArchitect = async () => {
  //   if (!architectMobile) return;
  //   try {
  //     // Call HRMS API with mobile number
  //     const response = await Digit.Hooks.hrms.useHRMSSearch(
  //       { userName: architectMobile },
  //       tenantId
  //     );
  //     console.log("HRMS response:", response);
  //     // You can handle the response here, e.g., save selected architect in sessionStorage
  //     sessionStorage.setItem("selectedArchitect", response);
  //   } catch (err) {
  //     console.error("Error fetching architect:", err);
  //   }
  // };
  // const {
  //   isLoading: hrmsLoading,
  //   isError,
  //   error: hrmsError,
  //   data: hrmsData,
  //   ...rest
  // } = Digit.Hooks.hrms.useHRMSSearch({ role: "BPA_ARCHITECT" }, tenantId, null, isupdate);

  // const architectOptions = useMemo(() => {
  //   if (!hrmsData?.Employees) return [];

  //   return hrmsData.Employees.map(emp => {
  //     const name = emp.user?.name || emp.code; // fallback to code if user info is missing
  //     const key = emp.user?.uuid || emp.code; // use UUID for value
  //     return { name, key };
  //   });
  // }, [hrmsData]);
  ...
  //   sessionStorage.setItem("selectedArchitect", JSON.stringify(architect));
  // };
  ...
  if (pdfLoading || isLoading || hrmsLoading) {
    return <Loader />
  }

  // const architectOptions = [
  //   { key: "ARCH001", name: "John Doe" },
  //   { key: "ARCH002", name: "Jane Smith" },
  //   { key: "ARCH003", name: "Rahul Kumar" },
  // ];
  ...
  return (
    <React.Fragment>
      ...
      <Card style={{ paddingRight: "16px" }}>
        <CardSubHeader>{t("BPA_SELECT_ARCHITECT")}</CardSubHeader>
        {/* <Dropdown
          t={t}
          option={architectOptions}
          optionKey="name"
          isMandatory={true}
          selected={selectedArchitect}
          select={handleArchitectSelect}
          placeholder={t("BPA_SELECT_ARCHITECT_PLACEHOLDER")}
        /> */}
        {/* <div style={{ marginTop: "30px" }}>
          <div className="field-container">
            <div style={{ position: "relative", zIndex: "100", left: "35px", marginTop: "-24.5px", marginLeft: "-25px" }}>+91</div>
            <TextInput
              ...
              isMandatory={true}
              optionKey="i18nKey"
              name="architectMobileInput"
              value={architectMobileInput}
              onChange={(e) => setArchitectMobileInput(e.target.value)}
              // {...(validation = {
              //     isRequired: true,
              //     pattern: "[6-9]{1}[0-9]{9}",
              //     type: "tel",
              ...
              disabled={false}
            />
            <div style={{ position: "relative", zIndex: "100", right: "35px", marginTop: "-24px", marginRight: "-20px" }} onClick={handleSearchArchitect}> <SearchIcon /> </div>
          </div>
          {hrmsData?.user?.length > 0 && (
            <div style={{ marginTop: "0px", fontWeight: "500", color: "#505A5F", marginBottom: "20px" }}>
              {t("Selected Architect")}: {hrmsData.user[0].name}
            </div>
          )}
        </div> */}
