...
export const OBPSService = {
  ...
  BPADetailsPage: async (tenantId, filters) => {
    ...
    else if (BPA?.businessService === "BPA"||BPA?.businessService === "BPA-PAP"||BPA?.businessService === "BPAC1") appBusinessService = ["BPA.NC_APP_FEE", "BPA.NC_SAN_FEE"];
    else if (BPA?.businessService === "BPA_OC") appBusinessService = ["BPA.NC_OC_APP_FEE", "BPA.NC_OC_SAN_FEE"];

    let fetchBillRes = {};

    if (appBusinessService?.[0]) {
      fetchBillRes = await Digit.PaymentService.fetchBill(BPA?.tenantId, { consumerCode: BPA?.applicationNo, businessService: appBusinessService[0] });
    }
